...
const TutorDashboard = () => {
  ...
  const handleAccept = async (id) => {
    try {
      const res = await axios.post(`/sessions/${id}/accept`, { message: decisionModal.message || undefined });
  ...
  const handleReject = async (id) => {
    try {
      const res = await axios.post(`/sessions/${id}/reject`, { message: decisionModal.message || undefined });
      setSessions(prev => prev.map(s => (s._id === id ? res.data : s)));
    } catch (e) {
      console.error(e);
  ...
  return (
    <div style={{ margin: '20px', backgroundColor: '#f0f8ff', minHeight: '100vh', padding: '20px' }}>
      ...
      <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '8px', marginBottom: '20px' }}>
        ...
        <form onSubmit={handleSaveAvailability} style={{ display: 'grid', gridTemplateColumns: '1fr 2fr auto', gap: '10px', alignItems: 'end' }}>
          <div>
            <label>Weekday</label>
            <select value={weekday} onChange={(e) => setWeekday(e.target.value)} style={{ width: '100%', padding: 8, borderRadius: 6, border: '1px solid #ccc' }}>
          ...
          <div>
            ...
            <input value={slotsText} onChange={(e) => setSlotsText(e.target.value)} style={{ width: '100%', padding: 8, borderRadius: 6, border: '1px solid #ccc' }} />
          </div>
          <button type="submit" style={{ backgroundColor: '#007bff', color: 'white', padding: '10px 16px', border: 'none', borderRadius: '8px' }}>Save</button>
        </form>
      ...
      <div style={{ backgroundColor: 'white', padding: 20, borderRadius: 12, marginBottom: 20, boxShadow: '0 6px 24px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <h3 style={{ margin: 0 }}>Notifications</h3>
          <span style={{ fontSize: 13, color: '#6c757d' }}>{notifications.filter(n => !n.read).length} unread</span>
        </div>
              ...
              >
                ...
                {!n.read && (
                  <button
                    ...
                    disabled={isMarking}
                    style={{ backgroundColor: '#007bff', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 10px' }}
                  >
                    Mark read
                  </button>
                )}
          ...
          <ul style={{ listStyle: 'none', padding: '0' }}>
            {sessions.map((session) => (
              <li key={session._id} style={{ marginBottom: '10px', padding: '10px', border: '1px solid #ddd', borderRadius: '5px' }}>
                ...
                <strong>Client Name:</strong> {session.client?.name || 'N/A'} <br />
                <strong>Status:</strong> {session.status || 'pending'}
                {session.status === 'pending' && (
                  <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                    <button onClick={() => openDecision('accept', session._id)} style={{ backgroundColor: '#28a745', color: 'white', padding: '8px 12px', border: 'none', borderRadius: 6 }}>Accept</button>
                    <button onClick={() => openDecision('reject', session._id)} style={{ backgroundColor: '#dc3545', color: 'white', padding: '8px 12px', border: 'none', borderRadius: 6 }}>Reject</button>
                  </div>
                )}
              </li>
            ))}
      ...
      {decisionModal.open && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ width: 'min(520px, 92vw)', background: '#fff', borderRadius: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
            <div style={{ padding: 16, borderBottom: '1px solid #eef1f4', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <strong style={{ fontSize: 18 }}>{decisionModal.action === 'accept' ? 'Accept Session' : 'Reject Session'}</strong>
              <button onClick={closeDecision} style={{ background: 'transparent', border: 'none', fontSize: 20, lineHeight: 1, cursor: 'pointer' }}>×</button>
            </div>
            <div style={{ padding: 16 }}>
              <label style={{ display: 'block', marginBottom: 8 }}>Optional message to client</label>
              <textarea
                value={decisionModal.message}
                onChange={(e) => setDecisionModal(prev => ({ ...prev, message: e.target.value }))}
                rows={4}
                placeholder={decisionModal.action === 'accept' ? 'e.g., Looking forward to our session!' : 'e.g., I’m unavailable at that time, please pick another slot.'}
                style={{ width: '100%', border: '1px solid #d7dce3', borderRadius: 10, padding: 10, background: '#fbfcfe' }}
              />
            </div>
            <div style={{ padding: 16, borderTop: '1px solid #eef1f4', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={closeDecision} style={{ background: '#6c757d', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 14px' }}>Cancel</button>
              <button onClick={submitDecision} style={{ background: decisionModal.action === 'accept' ? '#28a745' : '#dc3545', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 14px' }}>
