...
const bookSession = async (req, res) => {
  ...
  try {
    ...
    const availability = await Availability.findOne({ tutor: tutorId, weekday });
    const isAvailable = availability?.slots?.some(s => startMinutes >= s.startMinutes && endMinutes <= s.endMinutes);
    if (!isAvailable) {
      return res.status(400).json({ msg: 'Tutor not available at the requested time' });
    }
...
const acceptSession = async (req, res) => {
  try {
    ...
    const session = await Session.findByIdAndUpdate(id, { status: 'accepted' }, { new: true });
...
module.exports = { bookSession, getSessions, acceptSession, rejectSession, cancelSession };
